...
- DMM registers a non-primary MMPC tool entry so the missing tool is visible in extension capabilities.
...
- MMPC Modding Tool archive installation is blocked until DMM can generate `assetArchiveDir.txt` from the live game path and execute the Windows tool safely.
