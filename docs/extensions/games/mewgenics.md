...
- Registers `launch.bat` as the extension primary launch tool for enabled Mewgenics mods, including Vortex's `shell`, `detach`, and `exclusive` tool metadata.
...
- Live-test Steam Deck launch behavior for generated `launch.bat`.
- DMM now models Vortex's shell launch-tool metadata, but the current Steam launch-option bridge intentionally refuses to auto-configure `.bat`/`.cmd`/`.ps1` shell tools until a source-verified Proton/script execution bridge exists.
