...
- Related Steam AppIDs now covered by source-verified VPK routing:
...
- Live Steam Deck `gameinfo.txt` search-path verification:
  - `hl2/gameinfo.txt` declares `hl2/custom/*`
  - `lostcoast/gameinfo.txt` declares `lostcoast/custom/*` and `hl2/custom/*`
  - `episodic/gameinfo.txt` declares `episodic/custom/*` and `hl2/custom/*`
  - `ep2/gameinfo.txt` declares `ep2/custom/*`, `episodic/custom/*`, and `hl2/custom/*`

## Current DMM Capability

- DMM declares the verified Half-Life 2 Nexus domain `halflife2`.
- DMM registers the source-verified Steam AppID `220` through the base Half-Life 2 extension.
- Lost Coast, Episode One, and Episode Two are registered as separate source-backed Source-engine targets that reuse the Vortex Half-Life 2 `.vpk` installer shape but write into the episode-specific `custom` root verified from each installed `gameinfo.txt`.
- DMM checks for `hl2/gameinfo.txt` plus a native Linux or Windows executable marker.
- DMM installs `.vpk` archives through the source-verified Vortex installer shape into:
  - Half-Life 2: `hl2/custom`
  - Lost Coast: `lostcoast/custom`
  - Episode One: `episodic/custom`
  - Episode Two: `ep2/custom`
...
- Validate representative Lost Coast/Episode One/Episode Two archives if suitable Nexus files exist.
