...
| `bake-settings` | local game settings | Missing request/response bus | Profile-local settings file swap is implemented, but Vortex's mod-provided settings bake bus remains missing. |
...
- Missing a generic request/response bus equivalent to Vortex `emitAndAwait` for extension-to-core operations such as `browse-for-download` and `bake-settings`. `deploy-single-mod`, `purge-mods-in-path`, `discover-tools`, purge state-migration execution, and manifest `set-mod-type` migration execution have typed DMM runtime primitives, but broad arbitrary command-bus dispatch and state/profile migration execution are still pending.
...
- Has source-backed blocked metadata for new-file adoption/monitoring, Vortex/NMM/MO import UI entries, savegame profile features, and local game settings baking. Profile-local settings file sync itself is implemented through game extensions that declare the Vortex `local_game_settings` files.
