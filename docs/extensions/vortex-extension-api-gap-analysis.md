...
- Source-backed helpers now exist for Vortex `dazip` outer/inner archive planning/AddIns generation and UMM game opt-in metadata. Source-backed metadata remains blocked for shared Vortex mod types `dinput`, `enb`, and `gedosato`, plus DAZIP migration purge runtime and UMM external tool execution.
...
- Needed shared mod-type helpers include DAZIP migration purge runtime, executable-relative DLL deployment with unsafe-file confirmation, ENB game-root deployment, GeDoSaTo external tool discovery and texture targeting, and Unity Mod Manager external-tool download/discovery/patch execution.
