...
- Blade & Sorcery is promoted from catalog metadata into `internal/extensions/bladeandsorcery`, covering Vortex official `manifest.json` installer routing, engine-injection routing through `dinput`, obsolete MulleDK19 `mod.json` blocking, supported VR tools, setup metadata, and generated `StreamingAssets/Mods/loadorder.json` through the reusable JSON load-order hook. Source-backed drag/drop load-order UI, migration, and version-validation behavior remains blocked metadata.
...
- Vortex `modtype-umm`'s `ummAddGame` API now maps to a typed DMM `umm` helper for converted first-party Go extensions. DMM also supports the source-backed `umm-installer` archive shape as a managed `tool-only` staging payload. Tool discovery reports staged DMM-managed tool payloads and extension-declared game-root tools through `/api/games/{appID}/tools`; ready executable tools, including installed managed tool archives, can be launched by Decky through queued extension-tool actions. Runtime Unity Mod Manager auto-download, environment-aware setup, and noninteractive patch execution are still blocked.
