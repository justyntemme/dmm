...
| `purge-mods-in-path` | DAZIP, game migrations, Sims, Spyro, Blade & Sorcery | Partial runtime | DMM now has a scoped purge primitive that removes only latest-manifest-owned files under an absolute path with optional mod-type filtering, runs purge lifecycle hooks, and records a new current deployment manifest. Migration executor wiring remains separate work. |
...
| `discover-tools` | script extender installer | Typed DMM primitive | Reports extension-declared game-root tools and DMM-managed tool/script-extender payloads through `/api/games/{appID}/tools`; acquisition and launch execution remain separate capabilities. |
...
- Needed shared mod-type helpers include DAZIP migration purge runtime, DInput unsafe-DLL confirmation UI, ENB game-root deployment, GeDoSaTo texture targeting, and Unity Mod Manager external-tool download/launch/patch execution.
...
- Missing a generic request/response bus equivalent to Vortex `emitAndAwait` for extension-to-core operations such as `browse-for-download` and `bake-settings`. `deploy-single-mod`, `purge-mods-in-path`, and `discover-tools` have typed DMM runtime primitives, but broad command-bus dispatch and migration execution are still pending.
