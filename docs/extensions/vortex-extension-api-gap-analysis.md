...
| `context.registerModType` | 40 | `RegisterModType` | Partial runtime | Source-backed shared helpers cover DInput trust confirmation, GeDoSaTo texture targeting, UMM tool/runtime acquisition, and ENB's active game-root mod-type registration. The upstream ENB automatic installer remains commented out, so DMM must not invent installer behavior for it. |
...
| `context.registerMigration` | 18 | `RegisterStateMigration` | Partial runtime | DMM executes extension-declared safe purge commands, manifest `set-mod-type` retag/retarget commands, and active-profile redeploy commands once per game/migration, gated by the previous persisted extension snapshot so historical migrations do not run on fresh installs. Ready migrations must declare executable commands; historical Vortex-only state rewrites are marked not-applicable to DMM-created state until the post-MVP Vortex import wizard implements real detection/repair. Adoption migrations and profile/settings-state migrations remain pending. |
| `context.registerDialog` | 12 | `RegisterExtensionDialog` | Partial runtime | Source-backed dependency-manager connector/editor/conflict/file-override dialogs map to DMM profile rule APIs, file-winner conflict APIs, deploy preview, and Action Center conflict prompts. The dependency cycle graph is verified non-applicable to DMM-created state because cyclic before/after rules are rejected when saved; imported legacy/Vortex rule state remains a post-MVP repair-wizard concern. Remaining dialogs still need either a typed product surface or a verified non-applicable decision. |
...
| `context.registerTableAttribute` | 9 | `RegisterExtensionTableAttribute` | Partial runtime | Source-backed concrete attributes are represented by typed DMM surfaces, including Gamebryo index locks, dependency-rule/load-order badges, and Vortex mod-content classification exposed through installed mod APIs. The unbound generic desktop table placeholder was removed so DMM does not advertise non-executable columns. |
...
| `context.registerSettings` | 8 | `RegisterExtensionSetting` | Partial runtime | Generic value persistence/API/events, typed JSON/string/path/bool/number schema validation, default values, and phone/tablet rendering are implemented for registered ready settings. Extension tests, target-root resolvers, and deployment/lifecycle events now receive defaults plus persisted overrides. Vortex `apply-settings` behavior for Gamebryo archive invalidation is represented through extension-declared profile-file INI patches, and Vortex's archive timestamp backdate test is represented through extension-declared diagnostics/repair for the supported Gamebryo archive prefixes. Dependency workarounds map to DMM's profile-scoped mod-rule settings. Remaining gaps are other concrete setting effects and richer Vortex-equivalent custom setting components. |
| `context.registerDashlet` | 7 | Source inventory only | Source-only | Vortex dashboard tiles are desktop product UI, not game-deployment runtime. DMM keeps source references for audit but no longer advertises generic dashlet capabilities. Concrete alerts belong in Action Center, diagnostics, or jobs. |
| `context.registerMainPage` | 7 | `RegisterExtensionMainPage` | Partial runtime | DMM maps source-backed pages to typed product surfaces where behavior exists: Gamebryo save games use the profile-savegames API/UI, and Morrowind plugins use the Morrowind plugin activation/load-order UI/API. Remaining generic extension pages need a phone/tablet page runtime or explicit non-applicable decisions. |
| `context.registerMerge` | 5 | `RegisterMerge` plus shared helpers and extension-owned deploy hooks | Partial runtime | Source-backed runtime exists for XML/MTL merges, DAZIP AddIns generation, MSFS aircraft.cfg/locPak merged packages, Witcher settings/config merges, and ARCtool-backed Dragon's Dogma ARC rebuilds. Remaining merge work is driven by concrete source-backed extensions that require new archive or patch helpers. |
...
| `context.registerArchiveType` | 4 | `RegisterArchiveType` plus native archive runtimes | Runtime | BSA/BA2 list/read/extract and BSA write/create are implemented. QuickBMS and ARCtool now have typed process bridges matching Vortex's external-helper model; Dragon's Dogma wires ARCtool through an extension-owned selective ARC merge hook. Source review confirms Vortex's BA2 handler is read/extract only and only the BSA handler supports create mode. |
...
| `context.optional.registerCollectionFeature` | 3 | `RegisterCollectionFeature` plus generic profile collection export/import endpoints | Partial runtime | DMM exports/imports installed profile mod identity, enabled state, and order for load-order collection features. Game-specific merged-data postprocessing such as Witcher 3 script-merge collection data remains pending. |
| `context.registerPersistor` | 3 | Gamebryo plugin activation plus LOOT metadata runtime | Runtime | The only source-backed use persists Gamebryo `loadOrder`, `userlist`, and `masterlist`. DMM maps this to profile plugin activation state, generated `plugins.txt`/`loadorder.txt`, per-profile LOOT userlist files, and masterlist refresh. The generic persistor placeholder was removed. |
| `context.registerLoadOrderPage` | 3 | `RegisterExtensionLoadOrderPage` | Partial runtime | Concrete game extensions register source-backed load-order pages where DMM has typed profile/order behavior, such as Conan Exiles, Microsoft Flight Simulator, Blade & Sorcery, Witcher 3, and BG3. The unbound generic desktop page placeholder was removed. |
| `context.registerAttributeExtractor` | 2 | `RegisterAttributeExtractor` plus install-plan metadata extractors | Partial runtime | Source-backed attributes can be extracted during extension-owned install planning, including Stardew SMAPI manifest metadata, Pillars II `SupportedGameVersion` ranges, and Morrowind ESP/ESM plugin-list metadata. A generic post-install extractor scheduler remains pending for future extensions that need to inspect already-installed unmanaged state instead of archive contents. |
| `context.registerGameInfoProvider` | 2 | `RegisterGameInfoProvider` | Runtime | Ready providers run through `/api/games/{appID}/info` with source-shaped priority/tags/cache metadata. DMM caches aggregate provider details for the minimum provider-declared TTL, keyed by app/path/build/game-version input, and provider detail rows render in the phone/tablet Review view plus the selected-game Decky surface. DMM mirrors Vortex's Steam game-info provider through the public Steam Store `appdetails` endpoint, so no Steam API key is required for this surface. |
...
| `context.registerActionCheck` | 2 | `RegisterExtensionActionCheck` plus typed LOOT validation | Partial runtime | Gamebryo duplicate userlist-rule validation is enforced in the LOOT userlist write path and exposed as a concrete source-backed action check. The Vortex Redux/persistor coordination guard does not map to DMM's backend-owned state model and is not advertised as a generic placeholder. |
| `context.registerControlWrapper` | 1 | `RegisterExtensionControlWrapper` | Runtime | Vortex's single source-backed control wrapper is the dependency-manager mod-name wrapper. DMM maps it to profile mod rows exposing rule/conflict badges, unresolved dependency notices, and file-conflict actions from the profile rule/conflict read models. |
| `context.registerHistoryStack` | 1 | `RegisterHistoryStack` plus profile deployment history | Runtime | Vortex's single source-backed use is the Gamebryo plugin history stack. DMM represents that with profile plugin activation state plus deployment history preview/restore after plugin enable/disable/order changes instead of copying Vortex's desktop undo stack. |
...
| `added-files` / `removed-files` | `new-file-monitor`, BattleTech | Runtime | DMM snapshots extension-owned target roots, detects added and removed unmanaged files before deployment, passes Vortex-style candidate owner lists to extension handlers, lets extensions adopt selected added files, persists adopted manifest entries, exposes removed-file deltas to extension handlers, and supports extension-declared external unmanaged file adoption into DMM-owned staging. Source review found BattleTech consumes `added-files`; no bundled Vortex extension consumes `removed-files`, so DMM's removed-file event emission matches the current source-backed runtime surface. |
...
| `will-install-dependencies` | dependency manager | Partial runtime | DMM can queue extension-declared runtime acquisitions through the shared captured-install pipeline from Deck activation, deploy guards, Decky UI, or phone/tablet Review. Generic dependency download resolution remains a follow-up where a source-verified provider exposes installable dependency metadata. |
...
| `update-conflicts-and-rules` | dependency manager | Runtime | `POST /api/games/{appID}/conflicts-and-rules/recompute` validates the game, runs any extension-owned handler with Vortex's `calculateOverrides` flag, republishes the normal deployment refresh event, and returns the same diagnostics/deploy-preview read model used by the UI. DMM deployment planning treats duplicate managed file targets as unresolved until a profile file-winner is selected, except where an extension declares a source-backed conflict-ignore pattern. Profile before/after/conflicts rules are persisted through profile rule APIs, checked for cycles, and normalized into profile priority order. Override redundancy repair and the full Vortex graph editor are UI/product follow-up work. |
...
| `discover-tools` | script extender installer, UMM | Typed DMM primitive | Reports extension-declared game-root tools and DMM-managed tool/script-extender payloads through `/api/games/{appID}/tools`; managed tools inherit default-primary and launch-argument metadata from matching extension declarations; ready executable tools can queue Decky-owned launch actions through `/api/games/{appID}/tools/{toolID}/launch`; extension-declared acquisition sources can queue captured installs through `/api/games/{appID}/tools/{toolID}/acquire` or browser handoffs through `/open-browser`; extension-declared environment variables flow into the Steam launch-options payload. Automated patch execution remains separate capability-specific work. |
...
| `unfulfilled-rules` | dependency manager | Runtime | DMM evaluates extension-extracted metadata dependencies for enabled profile mods, including required/recommended severity, minimum versions, disabled-mod handling, logical-name aliases, and extension-provided handlers that can mark an unfulfilled rule as handled. Duplicate managed file targets are surfaced as blocking conflicts until resolved through profile file-winner overrides or extension conflict-ignore metadata, and profile before/after/conflicts rules provide Vortex-style conflict ordering semantics with cycle rejection. |
...
- Dedicated source-backed `registerGame` ports with remaining blocked runtime gaps: `game-prisonarchitect` maps Vortex LocalAppData mod deployment to Proton LocalAppData and selects source-verified native Linux `~/.Prison Architect/mods` when the native executable is installed. `game-nehrim` now resolves Vortex's Nehrim-to-Oblivion cross-app install root through Steam app manifests and enables the `data` installer with copy deployment.
...
- Source-backed `registerGameStub` support-mod entries: none remain in the catalog. `game-cyberpunk2077`, `game-dmc5`, `game-mount-and-blade2`, `game-palworld`, `game-re2remake`, `game-re3remake`, and `game-starfield` now have dedicated DMM extension packages with Steam Deck app discovery, Nexus domain routing, and deployable source-declared game-root archive installers. `game-subnautica` and `game-subnauticabelowzero` are promoted with equivalent identity routing and deployable source-declared `QMods` archive-root installers.
...
6. Extend new-file monitoring only when a source-backed extension needs more than the current added-file adoption and removed-file event emission surfaces.
...
- Bloodstained: Ritual of the Night is promoted from catalog metadata into `internal/extensions/bloodstainedritualofthenight`, covering Vortex `.pak` archive handling, the `BloodstainedRotN/Content/Paks/~mods` target root, the alphabetic load-order prefix rewrite Vortex uses because the game loads paks by folder name, extension-declared unmanaged PAK adoption, and the legacy `~mod` migration purge.
- Code Vein is promoted from catalog metadata into `internal/extensions/codevein`, covering Vortex `.pak` archive handling, the `CodeVein/content/paks/~mods` target root, executable version metadata, the alphabetic load-order prefix rewrite Vortex uses because the game loads paks by folder name, and extension-declared unmanaged PAK adoption. Vortex's historical pre-1.0 load-order migration is verified non-applicable to DMM-created state because DMM has no released legacy Code Vein load-order state and already writes the current prefixed pak folder order.
...
- Star Wars: Knights of the Old Republic and KOTOR II are promoted from catalog metadata into `internal/extensions/kotor`, covering Vortex game-root folder installs, default override-folder installs, KOTOR II Steam launcher metadata, and managed TSLPatcher utility/mod staging with an explicit post-deploy launch-tool action.
...
- Dragon's Dogma is promoted from catalog metadata into `internal/extensions/dragonsdogma`, covering Vortex nativePC archive routing, the Vortex invalid-archive confirmation installer, selective MT Framework ARC merging for `game_main.arc` and `title.arc` through an extension-owned restore-aware `will-deploy` hook backed by the shared ARCtool bridge, and the source-backed 1.0.1 historical staged-mod migration through DMM's executable `move-staged-paths` migration command.
- Blade & Sorcery is promoted from catalog metadata into `internal/extensions/bladeandsorcery`, covering Vortex official `manifest.json` installer routing, engine-injection routing through `dinput`, obsolete MulleDK19 `mod.json` blocking, supported VR tools, setup metadata, generated `StreamingAssets/Mods/loadorder.json` through the reusable JSON load-order hook, a typed open-directory action for the official mods folder, source-backed `Game.json`/`global.json` version detection, `manifest.json` `GameVersion` compatibility metadata, and unmanaged external refresh through generic marker-folder adoption for manually added manifest folders. Source-backed drag/drop load-order UI is represented by DMM's profile-order API instead of copied as a desktop table, and historical version-conditional migrations are not applicable to DMM-created state unless post-MVP Vortex import is enabled.
...
- Needed helper APIs include source-backed versions of Vortex `testSupportedContent`, broader wrapper-root normalization variants, and richer component-choice rules.
- Needed shared mod-type helpers include any future source-backed Unity Mod Manager command contract if UMM exposes one beyond its own UI. ENB has no active upstream automatic installer in the verified Vortex source; DMM parity is the game-root mod-type primitive until a source-backed installer path exists.
...
- Source-backed desktop-only Vortex UI surfaces such as dashlets, theme pages, feedback pages, documentation pages, and import dialogs are now kept as source inventory only when they do not affect DMM-created deployment/runtime state. Concrete source-backed behavior remains modeled through typed runtime surfaces: actions, settings, tests, health checks, dependency dialogs, load-order pages, profile files, reducers, table attributes, Gamebryo/LOOT persistence, and startup hooks. `mod-content` is backed by DMM's staged-manifest content classifier, and `mod-report` is backed by DMM's JSON/text mod report endpoint. `registerStartHook` is no longer tracked as a generic UI/state blocker because the only verified source-backed use is the dependency-manager startup conflict check represented by a typed runtime. `registerTest` now has a typed diagnostics runtime for extension-owned checks, `gamemode-activated` tests run through the Decky active-game report path, and Gamebryo `apply-settings` INI edits are represented as extension-declared profile-file patches. Other test triggers and arbitrary UI/state callbacks remain pending only when a concrete source-backed runtime need is identified.
- DMM should not add a generic renderer/executor for arbitrary Vortex desktop React callbacks. Converted extensions may expose concrete typed actions, dialogs, pages, and attributes only when DMM has equivalent product behavior; otherwise the source remains inventory-only or the feature stays explicitly blocked until a typed runtime is designed.
...
- Source-backed metadata exists for shared-system APIs/events used by FNIS, local game settings, dependency management, new-file monitoring, and Vortex test helpers: `deploy-single-mod`, `purge-mods-in-path`, `browse-for-download`, `nexus-download`, `discover-tools`, `bake-settings`, `unfulfilled-rules`, `registerGameInfoProvider`, new-file adoption, removed-file reporting, and extension-declared external unmanaged file adoption. `browse-for-download`, `nexus-download`, `deploy-single-mod`, `purge-mods-in-path`, `discover-tools`, `bake-settings`, `unfulfilled-rules` dependency override handlers, Decky extension-tool launch actions, profile metadata dependency evaluation, removed-file reporting, purge state-migration execution, manifest `set-mod-type` retag/retarget migration execution, migration active-profile redeploy, wait-for-tool-exit actions, backend-prepared tool input files, generated profile-mod recording/deploy, and external unmanaged file adoption are backed by implemented typed runtime primitives. FNIS now has a reusable source-backed helper for supported-game opt-in, settings, patch-list parsing, installed/version diagnostics, automatic generator launch, profile patch input writing, and generated profile output deployment. The others remain metadata-only or blocked unless separately noted.
- Missing runtime implementations for broader migration behavior equivalent to Vortex state-rewriting `registerMigration` handlers and any future concrete reducer/state surface that cannot be represented by existing typed settings/profile/plugin/load-order stores. DMM rejects ready migrations with no executable commands so source-backed historical Vortex migrations cannot be accidentally advertised as runtime parity. The generic `registerPersistor` gap was removed because its only verified use is covered by DMM's Gamebryo/LOOT runtime.
