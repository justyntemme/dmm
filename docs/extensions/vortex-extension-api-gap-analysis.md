...
| `context.registerMigration` | 18 | `RegisterStateMigration` | Partial runtime | DMM executes extension-declared safe purge commands and manifest `set-mod-type` commands once per game/migration. Old-extension-version gating, adoption migrations, and profile/settings-state migrations remain pending. |
...
3. Add more migration command runtimes for source-backed migrations that rewrite load-order state, adopt unmanaged files, or migrate profile/settings data. Manifest `set-mod-type` retagging is implemented, but old-extension-version gating is still required before enabling source-backed historical migrations such as No Man's Sky on real installs.
