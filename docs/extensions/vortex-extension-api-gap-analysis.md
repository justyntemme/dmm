...
- Source-backed helpers now exist for Vortex `dazip` outer/inner archive planning/AddIns generation and UMM game opt-in plus tool archive staging metadata. Source-backed metadata remains blocked for shared Vortex mod types `dinput`, `enb`, and `gedosato`, plus DAZIP migration purge runtime and UMM external tool execution.
