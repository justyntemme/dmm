...
- Missing a generic request/response bus equivalent to Vortex `emitAndAwait` for extension-to-core operations such as `browse-for-download`, `discover-tools`, and `bake-settings`. `deploy-single-mod` and `purge-mods-in-path` have typed DMM runtime primitives, but broad command-bus dispatch and migration execution are still pending.
