...
- `game-7daystodie/src/index.tsx` prompts for a User Data Folder, writes launcher settings, and deploys modlets under that selected folder's `Mods` directory. DMM now gives target-root resolvers access to persisted extension setting values, exposes a ready 7DTD User Data Folder setting, and routes 7DTD modlet deployment through an extension-declared target root that resolves either the selected UDF `Mods` folder or Vortex's game-root fallback. Generating Vortex's `launchersettings.json` from the selected UDF remains a separate setup action gap.
