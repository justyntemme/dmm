...
| `context.registerGameInfoProvider` | 2 | `RegisterGameInfoProvider` | Partial runtime | Ready providers run through `/api/games/{appID}/info` with source-shaped priority/tags/cache metadata. DMM now caches aggregate provider details for the minimum provider-declared TTL, keyed by app/path/build/game-version input, and provider detail rows render in the phone/tablet Review view plus the selected-game Decky surface. Remaining gap is a Steam Web API detail provider. |
