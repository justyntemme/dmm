...
- Source-backed helpers now exist for Vortex `dazip` outer/inner archive planning/AddIns generation, UMM game opt-in plus tool archive staging/acquisition metadata, and the shared Vortex `dinput` installer/mod type. DInput archives now route files beside the extension-declared game executable, matching Vortex's executable-relative deployment behavior. ENB and GeDoSaTo remain blocked metadata, plus UMM patch execution.
...
- Needed shared mod-type helpers include DInput unsafe-DLL confirmation UI, ENB game-root deployment, GeDoSaTo texture targeting, and Unity Mod Manager environment-aware setup/patch execution.
