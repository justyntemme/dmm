...
| `context.registerDialog` | 12 | `RegisterExtensionDialog` | Partial runtime | Source-backed dependency-manager connector/editor/conflict/file-override dialogs map to DMM profile rule APIs, file-winner conflict APIs, deploy preview, and Action Center conflict prompts. Remaining dialogs still need either a typed product surface or a verified non-applicable decision; the dependency cycle graph is blocked until DMM has a cycle visualization/editor. |
