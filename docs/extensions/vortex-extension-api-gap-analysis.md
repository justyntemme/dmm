...
- Still missing execution points for `removed-files` and `update-conflicts-and-rules`; `gamemode-activated`, `will-install-dependencies`, and `check-mods-version` have typed partial runtimes but not broad Vortex request/response parity.
