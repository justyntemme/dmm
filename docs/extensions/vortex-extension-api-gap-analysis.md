...
| `context.registerModType` | 40 | `RegisterModType` | Partial runtime | Shared mod-type helpers still missing ENB runtime, DInput confirmation UI, and UMM patch execution. GeDoSaTo texture targeting is implemented for opt-in game extensions with explicit tool-path configuration. |
...
- Source-backed helpers now exist for Vortex `dazip` outer/inner archive planning/AddIns generation, UMM game opt-in plus tool archive staging/acquisition metadata, the shared Vortex `dinput` installer/mod type, and the Vortex GeDoSaTo all-texture installer/texture-root behavior. DInput archives now route files beside the extension-declared game executable, matching Vortex's executable-relative deployment behavior. Dark Souls II opts into GeDoSaTo texture deployment with `textures/DarkSoulsII` target-root mapping. ENB remains metadata because the upstream Vortex installer registration is commented out; UMM patch execution remains blocked.
...
- Needed shared mod-type helpers include DInput unsafe-DLL confirmation UI, any future source-backed ENB runtime path, and Unity Mod Manager environment-aware setup/patch execution.
