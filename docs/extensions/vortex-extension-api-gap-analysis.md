...
- Extension-declared Vortex `registerGame` metadata now covers executable path, preferred executable variants selected from files present under the game root, required files, static or dynamic query-mod-path signals, merge mode, cleanup requirement, stop patterns, compatible download domains, and environment key/value metadata.
