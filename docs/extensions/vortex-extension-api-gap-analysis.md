...
- Missing a generic request/response bus equivalent to Vortex `emitAndAwait` for extension-to-core operations such as `deploy-single-mod`, `browse-for-download`, `discover-tools`, and `bake-settings`. `purge-mods-in-path` has a typed DMM runtime primitive, but broad command-bus dispatch and migration execution are still pending.
