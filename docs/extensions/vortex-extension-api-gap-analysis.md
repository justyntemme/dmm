...
- Source-backed `registerGameStub` support-mod entries: none remain in the catalog. `game-cyberpunk2077`, `game-dmc5`, `game-mount-and-blade2`, `game-palworld`, `game-re2remake`, `game-re3remake`, and `game-starfield` now have dedicated DMM extension packages that preserve Vortex support-mod metadata without claiming installer support. `game-subnautica` and `game-subnauticabelowzero` are promoted further with Steam Deck app discovery, Nexus domain routing, and deployable source-declared `QMods` archive-root installers.
