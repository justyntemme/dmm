...
func registerSupportedTools(r sdk.Registrar) {
	for _, tool := range []sdk.SupportedToolSpec{
		...
		},
		{
			...
			Status:                sdk.CapabilityStatusReady,
			Message:               "Vortex discovers XCOM 2 SDK through Steam app 299990. DMM resolves the tool from that app's Steam install root.",
		},
		{
		...
		},
		{
			...
			InstallRootSteamAppID: "602410",
			ExecutableRelative:    "Binaries/Win32/ModBuddy/XCOM ModBuddy.exe",
			RequiredFiles:         []string{"Binaries/Win32/ModBuddy/XCOM ModBuddy.exe"},
			Status:                sdk.CapabilityStatusReady,
			Message:               "Vortex discovers WOTC SDK through Steam app 602410. DMM resolves the tool from that app's Steam install root.",
		},
	} {
