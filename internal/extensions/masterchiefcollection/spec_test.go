...
func TestExtensionRegistersGameAndCapabilities(t *testing.T) {
	...
	if len(summary.Capabilities.Installers) != 3 || len(summary.Capabilities.EventHandlers) != 3 || len(summary.Capabilities.LaunchTools) != 1 || len(summary.Capabilities.SupportedTools) != 1 || len(summary.Capabilities.LauncherRequirements) != 2 || len(summary.Capabilities.ExtensionTests) != 1 {
