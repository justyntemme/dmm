...
func TestExtensionRegistersBepInExInstallerSupport(t *testing.T) {
	...
	gamePath := t.TempDir()
	writeFile(t, filepath.Join(gamePath, "DaveTheDiver.exe"), "exe")
	writeFile(t, filepath.Join(gamePath, "UnityPlayer.dll"), "dll")
	writeFile(t, filepath.Join(gamePath, "GameAssembly.dll"), "dll")
	writeFile(t, filepath.Join(gamePath, "DaveTheDiver_Data", "globalgamemanagers"), "data")
...
func TestBepInExPluginInstallerTargetsPluginsFolder(t *testing.T) {
	root := t.TempDir()
	writeFile(t, filepath.Join(root, "Wrapper", "DaveMod.dll"), "plugin")
	...
	assertTarget(t, plan, "BepInEx/plugins/DaveMod.dll")
