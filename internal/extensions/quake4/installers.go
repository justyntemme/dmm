...
func buildFSGameArchive(input installplan.BuildInput) (installplan.Plan, error) {
	files, err := listFiles(input.ExtractedRoot)
	if err != nil {
		return installplan.Plan{}, err
	}
	rootRel := fsGameRoot(files)
	if rootRel == "" {
		return installplan.Plan{}, installplan.Unsupported("Quake 4 archive does not contain a supported fs_game folder layout")
	}
	plan := installplan.Plan{
		GameID:     input.GameID,
		ModType:    input.Installer.ModType,
		...
		NameSource: installplan.NameSourceArchive,
		DetectedFrom: []installplan.Detection{{
			Kind:   "idtech4-fs-game",
			Path:   rootRel,
			Reason: "Verified Quake 4 fs_game archives copy one mod folder beside q4base and launch with +set fs_game <folder>",
		}},
	}
	for _, file := range files {
		if !pathWithinRoot(file, rootRel) {
		...
		targetRel := filepath.ToSlash(file)
		if targetRel == "" {
			continue
		}
		plan.Instructions = append(plan.Instructions, installplan.Instruction{
	...
	if len(plan.Instructions) == 0 {
		return installplan.Plan{}, errors.New("Quake 4 fs_game installer matched but produced no deployable files")
	}
	sort.SliceStable(plan.Instructions, func(i, j int) bool {
		return plan.Instructions[i].TargetRelative < plan.Instructions[j].TargetRelative
	})
