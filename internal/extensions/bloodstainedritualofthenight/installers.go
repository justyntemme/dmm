...
func buildPakArchive(input installplan.BuildInput) (installplan.Plan, error) {
	...
	if !ok {
		return installplan.Plan{}, installplan.Unsupported("Bloodstained archive does not contain a .pak file")
	}
	return simplearchive.BuildCopyPlan(
		input,
		contentRoot,
		input.TargetRoot,
		"vortex-bloodstainedrotn-pak",
		firstPakPath(files, contentRoot),
		"Vortex Bloodstained installer matched a .pak archive and copies files from the .pak folder into BloodstainedRotN/Content/Paks/~mods",
		"Bloodstained .pak installer matched but produced no deployable files",
		nil,
	)
}
