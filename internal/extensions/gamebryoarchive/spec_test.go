...
func TestExtensionRegistersSourceBackedArchiveMetadata(t *testing.T) {
	...
	if byID["ba2"].Engine != ID || byID["ba2"].Status != sdk.CapabilityStatusBlocked {
		t.Fatalf("ba2 capability = %+v", byID["ba2"])
	}
	if byID["bsa"].Engine != ID || !byID["bsa"].SupportsWrite || byID["bsa"].Status != sdk.CapabilityStatusBlocked {
