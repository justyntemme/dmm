...
func Register(r sdk.Registrar) {
	r.RegisterGame(sdk.GameRegistration{SteamAppIDs: []string{SteamAppID}, StoreAppIDs: map[string][]string{"gog": {GOGAppID}}, NexusDomains: []string{VortexGameID}, VortexGameID: VortexGameID, ExecutableRelative: executable, RequiredFiles: []string{executable}, QueryModPath: umm.ModRoot, MergeMode: sdk.GameMergeModeAll, Environment: map[string]string{"SteamAPPId": SteamAppID}, Deployment: installplan.DeploymentSpec{AllowNeedsReviewState: true}})
	umm.RegisterGameSupport(r, umm.GameOptions{GameID: VortexGameID, GameName: Name, AutoDownload: true})
	r.RegisterGameStore(sdk.GameStoreSpec{ID: "gog", Name: "GOG", Message: "Vortex can discover Pathfinder: Wrath of the Righteous through GOG app 1207187357. DMM supports this GOG identity through manual install-path registration; automatic GOG discovery is tracked separately."})
