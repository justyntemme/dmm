...
func TestASIArchivePlansToGameRoot(t *testing.T) {
	...
	writeFile(t, filepath.Join(root, "PrototypeFix.asi"), "asi")
	writeFile(t, filepath.Join(root, "PrototypeFix.ini"), "ini")
	...
	for _, want := range []string{"dinput8.dll", "PrototypeFix.asi", "PrototypeFix.ini"} {
		if !targets[want] {
...
func TestTexModPackagePlansToDMMTexModFolder(t *testing.T) {
	root := t.TempDir()
	writeFile(t, filepath.Join(root, "texture", "PrototypeSkin.tpf"), "tpf")
	writeFile(t, filepath.Join(root, "readme.txt"), "readme")
	...
	if plan.ModType != tpfModType || plan.PlannerID != "source:prototype:tpf" {
		t.Fatalf("plan = %+v", plan)
	}
	if len(plan.Instructions) != 1 || plan.Instructions[0].TargetRelative != "DMM/TexMod/Packages/PrototypeSkin.tpf" {
...
func TestTexModToolStagesManagedTool(t *testing.T) {
	...
	if plan.ModType != texmodToolType || plan.PlannerID != "source:prototype:texmod-tool" {
		t.Fatalf("plan = %+v", plan)
	}
	targets := map[string]bool{}
	for _, instruction := range plan.Instructions {
...
func TestDidDeployTexModQueuesManualToolAction(t *testing.T) {
	...
	if notice.ActionKind != sdk.EventNoticeActionRunLaunchTool || notice.ToolID != "prototype-texmod" || notice.AutoRun {
