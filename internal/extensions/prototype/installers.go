...
func buildTexModToolArchive(input installplan.BuildInput) (installplan.Plan, error) {
	...
	if !ok {
		return installplan.Plan{}, installplan.Unsupported("Prototype TexMod tool archive does not contain " + texmodExec)
	}
	plan := installplan.Plan{
		...
		DetectedFrom: []installplan.Detection{{
			Kind:   "prototype-texmod-tool",
			Path:   marker,
			Reason: "Prototype TexMod tool archives stage Texmod.exe under DMM/TexMod for extension-owned launch actions",
		}},
		Metadata: []installplan.ModMetadata{{
			Kind:            "tool",
			Name:            "TexMod",
			UniqueID:        "prototype-texmod",
			SourcePath:      marker,
	...
	if len(plan.Instructions) == 0 {
		return installplan.Plan{}, errors.New("Prototype TexMod tool installer matched but produced no deployable files")
	}
...
func buildTPFArchive(input installplan.BuildInput) (installplan.Plan, error) {
	...
	if len(tpfFiles) == 0 {
		return installplan.Plan{}, installplan.Unsupported("Prototype TexMod archive does not contain .tpf packages")
	}
	...
	plan := installplan.Plan{
		...
		NameSource: installplan.NameSourceArchive,
		DetectedFrom: []installplan.Detection{{
			Kind:   "prototype-texmod-package",
			Path:   strings.Join(tpfFiles, ","),
			Reason: "Prototype TexMod package archive matched .tpf files",
		}},
