...
func TestExtensionRegistersVortexRootInstaller(t *testing.T) {
	extension := gameext.MustCompileExtension(metalgearsolidmc.Extension())
	registry := gameext.NewRegistry([]gameext.Extension{extension})
	summary := registry.ExtensionSummaries()[0]
	if summary.ID != metalgearsolidmc.VortexGameID {
		t.Fatalf("summary id = %q", summary.ID)
	}
	if len(summary.SteamAppIDs) != 1 || summary.SteamAppIDs[0] != metalgearsolidmc.SteamAppID {
		t.Fatalf("steam app ids = %+v", summary.SteamAppIDs)
	}
	if len(summary.NexusDomains) != 1 || summary.NexusDomains[0] != metalgearsolidmc.VortexGameID {
...
func TestPlannerBuildsRootArchiveWithCommonWrapperStripped(t *testing.T) {
	...
	extension := gameext.MustCompileExtension(metalgearsolidmc.Extension())
	plan, err := gameext.NewRegistry([]gameext.Extension{extension}).BuildInstallPlan(metalgearsolidmc.SteamAppID, root)
	if err != nil {
		t.Fatal(err)
	}
	if plan.ModType != "metalgearsolidmc-root" || plan.PlannerID != "vortex:metalgearsolidmc:root" {
